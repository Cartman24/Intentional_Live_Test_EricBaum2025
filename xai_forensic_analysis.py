...
def generate_report(chat_results, session_results=None, user_info=None, api_keys=None):
    ...
    report.append(f"## SEC Whistleblower TCR #17447-419-783-054")
    report.append("")
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python3 xai_forensic_analysis.py <chat_log_file> [session_data_file]")
        print("  chat_log_file:    Path to xAI chat log JSON file")
        print("  session_data_file: Optional path to prod-mc-auth.json session data")
        sys.exit(1)
    ...
    print(f"[*] Total messages analyzed: {chat_results['total_messages']}")
    print(f"[*] Real names found: {sum(chat_results['real_names_found'].values())}")
    print(f"[*] Real companies found: {sum(chat_results['real_companies_found'].values())}")
    print(f"[*] Fabricated elements: {len(chat_results['fabrications_found'])}")
    print(f"[*] Confession markers: {len(chat_results['confession_exchanges'])}")
